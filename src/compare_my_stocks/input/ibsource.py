...
from common.simpleexceptioncontext import (
    SimpleExceptionContext,
...
class IBSourceRemGenerator:
    ...
    def generate(
        self,
        host=config.Sources.IBSource.HostIB,
        port=config.Sources.IBSource.PortIB,
        clientId=None,
        readonly=True,
    ):
...
class IBSourceRem:
    ...
    def do_connect(self):
        try:
            self.ib.connect(
                self._host, self._port, clientId=self._clientid, readonly=self._readonly
            )
            IBSourceRem.ConnectedME = self
            self.connected = True
    ...
    def reqHistoricalData(self, contract, enddate, td):
        ...
        if td > 365:
            ...
            bars = self.ib.reqHistoricalData(
                ...
                whatToShow=self.whatToShow(contract),
                useRTH=True,
            )
    ...
    def reqHistoricalData_ext(self, contract, enddate, td):
        ...
        bars = self.reqHistoricalData(cont, conv_date(enddate), td)
...
class IBSource(InputSource):
    ...
    @simple_exception_handling(
        err_description="get right contract bars ", return_succ=(None, None)
    )
    @cache_if_not_cond(cond=lambda x: x == (None, None))
    @cached(
        custom_key_maker=lambda self, contract, enddate, td: IBSource.get_contract(
            contract
        )
    ...
    def get_right_contract_bars(self, contract, enddate, td):
        with self.lock:
            ...
            if not cont["exchange"] and cont["secType"] in ["STK", "IND"]:
                logging.warn(
                    (f"(historicalhelper) warning: no exchange for contract {cont}")
                )
                cont["exchange"] = config.Symbols.TranslateExchanges.get(
                    cont["primaryExchange"], cont["primaryExchange"]
                )

            for i in range(2):
                try:
                    logging.debug(cont)
                    bars = self.ibrem.reqHistoricalData_ext(
                        cont, enddate.replace(hour=23), td
                    )  # we might get more than we opted for because it returns all the traded days up to the enddate..
                    return cont, bars
                except RequestError as e:
                    logging.debug(("req err", str(e), e.code))

                    if e.code in [WRONG_EXCHANGE, HISTORY_ERROR] or (
                        "enter exchange" in e.message.lower()
                    ):
                        if cont["conId"]:
                            ...
                            logging.debug(
                                (
                                    f"bad exchange for symbol {cont}. trying to  resolve with {config.Sources.IBSource.DefaultExchange}  . {e.message}"
                                )
                            )
                        ...
                        logging.debug(
                            (f"failed reqHistoricalData {e.message} {e.code}")
                        )
                        return None, None
    ...
    def historicalhelper(self, startdate, enddate, contract):
        ...
        try:
            is_cached = IBSource.get_right_contract_bars.cache_contains_argument(
                (self, contract, enddate, td)
            )
    ...
    def can_handle_dict(self, sym):
        ...
        return type(sym) == dict and (
            "validExchanges" in sym or "primaryExchange" in sym
        )
