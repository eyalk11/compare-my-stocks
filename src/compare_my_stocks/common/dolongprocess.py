...
class DoLongProcessSlots(QObject):
    ...
    def process_command(self, taskparams):
        ...
        realtask = simple_exception_handling(err_description="excpetion in real task",never_throw=True,err_to_ignore=[ValueError])(partial(self._task, *taskparams.params))
        self.started = True
        self.mutex.lock()
        try:
